...
def main() -> None:
    ...
    for _ in range(args.runs):
        for prompt in prompts:
            ...
            total_llm_calls += result.total_llm_stats["llm_calls"]
            total_prompt_tokens += result.total_llm_stats["prompt_tokens"]
            total_completion_tokens += result.total_llm_stats["completion_tokens"]
            total_total_tokens += result.total_llm_stats["total_tokens"]
            count += 1
    ...
    summary = {
        ...
        "avg_tokens_per_request": round(total_total_tokens / count, 2) if count else 0.0,
    }
